<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-input v-model="listQuery.username" placeholder="搜索发布单位" clearable style="width: 200px;margin-right: 15px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column label="发布单位" align="center" width="140">
        <template slot-scope="scope">
...
export default {
  ...
  data() {
    return {
      ...
      temp: {
        noticeContent: '',
        username: this.$store.state.teacher.userInfo.username,
  ...
  methods: {
    ...
    async handleFilter(){
      ...
      this.listLoading = true
      let result = await reqSearchNoticesList(this.listQuery.noticeContent, this.listQuery.username)
      console.log(result)
      if (result.status === 0){
        this.total = result.data.length
        this.list = result.data.filter((item, index) => index < this.listQuery.limit * this.listQuery.page && index >= this.listQuery.limit * (this.listQuery.page - 1))
    ...
    resetTemp(){
      ...
      console.log(this.$store.state.teacher.userInfo.username)
    },
    handleCreate(){
      this.resetTemp()
    ...
    async handleDeleteNotice(row) {
      let _id = row._id
      let result = await reqDeleteNotice(_id)
      if (result.status === 0){
        this.$message({
          message: result.msg,
          type: 'success'
