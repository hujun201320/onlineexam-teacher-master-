  ...
  <div class="login-container">
    <el-form ref="loginForm" :model="loginForm" :rules="loginRules" class="login-form" auto-complete="on" label-position="left">
      ...
      <el-form-item prop="username">
        ...
        <el-input v-model="loginForm.username" name="username" type="text" auto-complete="on" placeholder="请输入管理员姓名" />
      </el-form-item>
...
export default {
  ...
  data() {
    const validateUsername = (rule, value, callback) => {
      if (value.length < 2) {
        callback(new Error('请输入5位数字的教工号'))
      } else {
        callback()
      }
    }
  ...
  methods: {
    ...
    async teacherLogin() {
      const peoplename = this.loginForm.username
      const password = this.loginForm.password
      const result = await reqLogin({ peoplename, password })
      if (result.status === 0) {
        this.loading = false
        ...
        this.$message({
          message: `${result.data.peoplename}登录成功`,
          type: 'success'
        })
