<template>
  <div class="app-container">
    ...
    >
        ...
        <template slot-scope="scope">
          <el-form label-position="left" inline class="demo-table-expand">
            ...
            <el-form-item label="题目答案：">
              <span>{{ scope.row.multipleAnswer }}</span>
            </el-form-item>
      ...
      <el-table-column type="index" label="序号" prop="id" sortable align="center" width="80">
        <!-- <template slot-scope="scope">
  ...
  data() {
    return {
      ...
      listQuery: {
        page: 1,
        limit: 10,
        multiplecontent: undefined,
        langId: undefined,
        composeFlag: undefined
      },
      composeFlagOptions: [{ label: '是', key: '1' }, { label: '否', key: '0' }],
      langOptions: [{ label: '党员发展', key: '党员发展' }, { label: '政治理论', key: '政治理论' }, { label: '法规知识', key: '法规知识' }],
      temp: {
        content: '',
        pictureSrc: '',
        choiceA: '',
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      this.$nextTick(() => {
        ...
        this.$refs['dataForm'].model.multipleAnswer = row.multipleAnswer.split('')
      })
    },
    updateData() {
    ...
    async handleFilter() {
      ...
      const type = 'multiple'
      console.log(composeFlag)
      const result = await reqSearchMultipleList(this.listQuery.content, langId, composeFlag, type)
      if (result.status === 0) {
        this.total = result.data.length
        this.list = result.data.filter((item, index) => index < this.listQuery.limit * this.listQuery.page && index >= this.listQuery.limit * (this.listQuery.page - 1))
      }
    ...
    async handleDeleteQue(row) {
      const _id = row._id
      const result = await reqDeleteMultiple(_id)
      if (result.status === 0) {
        this.$message({
          message: result.msg,
          type: 'success'
