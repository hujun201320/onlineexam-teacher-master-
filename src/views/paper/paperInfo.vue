<template>
  <div class="app-container">
    ...
    >
      <el-table-column type="index" label="序号" prop="id" sortable align="center" width="90">
      ...
      <el-table-column prop="participateNum" sortable label="已参加人数" align="center" width="120">
        <template slot-scope="scope">
          <span>{{ scope.row.attendNum }}人</span>
        </template>
      </el-table-column>
      ...
      <el-form ref="fixedDataForm" :rules="fixRules" :model="temp" label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        <el-form-item label="所属科目" prop="langId">
          <el-select v-model="temp.langId" placeholder="选择科目" clearable style="width: 200px;margin-right: 15px;" class="filter-item" @change="fixedLangIdChange">
            <el-option v-for="item in langOptions" :key="item.key" :label="item.label" :value="item.langId" />
          </el-select>
        </el-form-item>
      ...
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        <el-form-item label="所属科目" prop="langId">
          <el-select v-model="temp.langId" placeholder="选择科目" clearable style="width: 200px;margin-right: 15px;" class="filter-item">
            <el-option v-for="item in langOptions" :key="item.key" :label="item.label" :value="item.label" />
          </el-select>
        </el-form-item>
        <el-form-item label="试卷名称" prop="paperName">
          <el-input v-model="temp.paperName" />
...
// import { mapGetters } from 'vuex'
  ...
  export default {
    ...
    data() {
      return {
        // idNumber:this.$store.state.teacherInfo.idNumber,
        tableKey: 0,
        list: null,
        total: 0,
        // listLoading: true,
        ...
        langOptions: [{ langId: '党员发展', label: '党员发展' }, { langId: '法规知识', label: '法规知识' }, { langId: '政治理论', label: '政治理论' }],
    ...
    methods: {
      async getList() {
        ...
        let result = await reqGetPapersList(idNumber)
        console.log(result)
        if (result.status === 0){
      ...
      async seePaperDetail(row, column, event) {
        ...
        const totalNum=row.singleNum.length + row.multipleNum.length + row.judgeNum.length + row.fillNum.length
        // const totalScore =row.fillScore*row.fillNum.length + row.multipleScore*row.multipleNum.length + row.judgeScore*row.judgeNum.length + row.singleScore*row.singleNum.length
        // this.totalNum = totalNum
        // this.totalScore = totalScore
        //初始化数据
        this.filterText = ''
        this.clickPaperTitle = `试卷详情：${row.paperName}`
        this.subPaperTitle = `试卷总分：${row.totalScore}分，试卷总题数：${totalNum}道。`
        this.minSubPaperTitle = `单选题${row.singleNum.length}道（每道${row.singleScore}分），多选题${row.multipleNum.length}道（每道${row.multipleScore}分），判断题${row.judgeNum.length}道（每道${row.judgeScore}分），填空题${row.fillNum.length}道（每道${row.fillScore}分）。`
        this.paperData = [{
          id: 1,
          label: '单选题列表',
          children: row.singleNum.map((item,index)=>{
            return {
              id:1,
          ...
          id: 2,
          label: '多选题列表',
          children: row.multipleNum.map((item,index)=>{
            return {
              id:1,
              label:index+1+':'+item.content
            }
          ...
          id: 3,
          label: '判断题列表',
          children: row.judgeNum.map((item,index)=>{
            return {
              id:1,
              label:index+1+':'+item.content
            }
          ...
          id: 4,
          label: '填空题列表',
          children: row.fillNum.map((item,index)=>{
            return {
              id:1,
              label:index+1+':'+item.content
            }
      ...
      async deletePaper(row) {
        let result = await reqDeletePaper(row._id)
        if (result.status === 0) {
          this.$message({
            message: '删除成功',
            type: 'success'
      ...
      async fixedLangIdChange() {
        ...
        if (this.temp.langId !== null) {
          const langId = this.temp.langId
          let result = await reqPaperQueDetailByLangId(langId)
          console.log(result)
          ...
          this.fixedPaperData = [{
            id: 1,
            label: '单选题列表',
            children: singleData.map((item, index)=>{
              return {
                id:1,
                label: item.content,
                _id: item._id,
            ...
            id: 2,
            label: '多选题列表',
            children: multipleData.map((item, index)=>{
              return {
                id:2,
                label: item.content,
                _id: item._id,
            ...
            id: 3,
            label: '判断题列表',
            children: judgeData.map((item, index)=>{
              return {
                id:3,
                label: item.content,
                _id: item._id,
            ...
            id: 4,
            label: '填空题列表',
            children: fillData.map((item, index)=>{
              return {
                id:4,
                label: item.content,
                _id: item._id,
          ...
          if (valid) {
            ...
            if (filtercheckedData.length > 0) {
              ...
              filtercheckedData.forEach((item, index) => {
                ...
                let restStr = item.item
                if (restStr!=='undefined') {
                  if (firstStr === '1') {
                  singleNum.push(restStr)
                }
                if (firstStr === '2') {
                  multipleNum.push(restStr)
                }
                if (firstStr === '3') {
                  judgeNum.push(restStr)
                }
                if (firstStr === '4') {
                  fillNum.push(restStr)
                }
                }
              })
              if (!singleNum.length) {
      ...
      async fixedInsertPaperInfo() {
        ...
        const paperType = 0
        let result = await reqFixedInsertPaperInfo(this.temp, paperType)
        if (result.status === 0){
      ...
      async randomInsertPaperInfo(){
        ...
        this.temp.paperDuration = parseInt(arr[0])*60*60 + parseInt(arr[1])*60
        const paperType = 1
        let result = await reqRandomInsertPaperInfo(this.temp, paperType)
        if (result.status === 0){
