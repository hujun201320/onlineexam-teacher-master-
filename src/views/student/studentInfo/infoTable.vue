<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.idNumber" placeholder="搜索身份证号" clearable style="width: 200px;margin-right: 15px;" class="filter-item" @keyup.enter.native="handleFilter" />
      <el-input v-model="listQuery.peoplename" placeholder="搜索姓名" clearable style="width: 200px;margin-right: 15px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    <el-table
      ...
      style="width: 100%;"
    >
      <el-table-column type="index" label="序号" prop="id" sortable align="center" width="120">
        <!-- <template slot-scope="scope">
          <span>{{ scope.row.id }}</span>
        </template> -->
      </el-table-column>
      <el-table-column label="身份证号" prop="sno" sortable align="center" width="140">
        <template slot-scope="scope">
      ...
      <el-table-column label="手机号" align="center" width="140">
        <template slot-scope="scope">
          <span>{{ scope.row.peoplePhone || '暂无手机号' }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="stuCreateTime" sortable label="注册时间" align="center" width="160">
        <template slot-scope="scope">
          <span>{{ scope.row.date_time | date-format }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="dialogFormVisible" title="添加">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="姓名" prop="peoplename">
          <el-input v-model="temp.peoplename" />
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        sno: [{ required: true, message: '身份证号为必填项', trigger: 'blur' },{ min: 18, max: 18, message: '身份证号必须为18位数字', trigger: 'blur' }],
  ...
  methods: {
    ...
    async handleFilter(){
      ...
      this.listLoading = true
      let result = await reqSearchStudentsList(this.listQuery.idNumber, this.listQuery.peoplename, this.listQuery.stuSex)
      if (result.statu === 0){
        this.total = result.data.length
        this.list = result.data.filter((item, index) => index < this.listQuery.limit * this.listQuery.page && index >= this.listQuery.limit * (this.listQuery.page - 1))
      }
    ...
    handleDownload(){
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['身份证号', '姓名', '性别', '邮箱', '手机号', '注册时间', '最近登录时间', '登录权限']
        const filterVal = ['idNumber', 'peoplename', 'stuSex', 'stuEmail', 'stuPhone', 'time', 'stuLastLoginTime', 'stuStatus']
