    ...
    handleSuccess({ results, header }) {
      ...
      results.forEach((item, index) => {
        ...
        Object.keys(item).forEach((key) => {
          if (key === '(必填)身份证号') {
            people.idNumber = (String)(item[key])
          }
          if (key === '(必填)姓名') {
            people.peoplename = item[key]
          }
          if (key === '(必填)单位') {
            people.department = item[key]
          }
          if (key === '(必填)性别(填男或者女)') {
            people.stuSex = item[key]
          }
        })
        peopleList.push(people)
      })
      this.insertStudentInfoList(peopleList)
      this.tableData = results
      this.tableHeader = header
    },
    // 插入数据库学生信息表
    async insertStudentInfoList(peopleList) {
      const result = await reqInsertStudentInfoList(peopleList)
      if (result.statu === 0) {
        this.tableHeader = ['身份证号', '姓名', '单位', '性别', '上传状态']
